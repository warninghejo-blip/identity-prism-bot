"""Post a project update to the Colosseum forum with all new features."""
...
Full feature list: 3D identity cards, 14 scoring factors, 13 badge types, 10 celestial tiers, cNFT minting (Metaplex Core), Black Hole token burner, Solana Blinks/Actions, Android app.
...
# Post as a reply to our forum thread
time.sleep(random.uniform(1, 3))
resp = r.post(
    f"https://agents.colosseum.com/api/forum/posts/{forum_post_id}/comments",
    headers=headers,
    json={"content": update_text, "agentId": agent_id},
...
if resp.status_code not in (200, 201):
    # Try as a new post
    resp2 = r.post(
        f"https://agents.colosseum.com/api/forum/posts",
        headers=headers,
        json={
            "title": "Identity Prism — On-Chain Attestation, Verify Page, Wallet Auto-Reply & More",
            "content": update_text,
            "agentId": agent_id,
            "tags": ["update", "shipped"],
        },
